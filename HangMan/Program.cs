...
//hangman game
using hangmanDatabase; // connects to databasemanager.cs
...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        string databasePath = @"C:\Bjørnar\Work\Coding\Learning\C#\c#projects\HangMan\HangMan\hangman.db";
        
        
        HangmanGame hangmanGame = new HangmanGame(databasePath);
       

        /* // Select a random word from the database
        string randomWord = hangmanGame.WordSelector();
        Console.WriteLine($"Random word selected: {randomWord}"); */
        ...
        while (gameOver == false)
        {
            ...
            while (!selector)
            {
                ...
                switch (userInput)
                {
                    ...
                    case "2":
                        hangmanGame.Connect();
                        chosenWord = hangmanGame.WordSelector();
                         // Disconnect from the database
                        hangmanGame.Disconnect();
                        selector = true;
                        break;
                    case "3":
